...
```ts
// ts 에서는 매개변수의 타입은 추론 못함
// 아래 처럼 기본값을 통해서 타입 추론
...
- 그런데 우리가 아는 것과 `반대로 생각해야 한다.` (매개변수 타입만은)
  : Super 타입과 Sub 타입이 있으면 Sub 타입은 Super 타입에 호환됩니다. 라고 배웠다.
  : 함수를 호환하는 경우는 반대로 생각하시길 바랍니다!;
- 호환 안되는 경우
